...
function Sandbox({
  ...
  const nodes = useMemo(
    () =>
      scenario.layout.nodes.map((node) => ({
        ...node,
  ...
  const edges = useMemo(
    () =>
      scenario.layout.edges.map((edge) => ({
        ...edge,
  ...
  const selectedEntity =
    playback.viewState.entities.find(
      (entity) => entity.id === selectedEntityId,
    ) ?? null;
  const highlightedNodeIds = nodes
    .filter((node) =>
      currentEvent?.relatedEntityIds.includes(node.entityId ?? node.id),
    )
    .map((node) => node.id);
  ...
  const handleEnterpriseSelect = (id: string) => {
    const node = nodes.find(
      (candidate) => candidate.businessCode === id || candidate.entityId === id,
    );
  ...
  return (
    <div className="app-shell">
      <ControlBar
        ...
        currentTime={new Date(currentTime).toLocaleString("zh-CN", {
          month: "2-digit",
          day: "2-digit",
          hour: "2-digit",
          minute: "2-digit",
      ...
      <main id="sandbox-main" className="workspace">
        <div
          className={
            mobileView === "enterprise"
              ? "mobile-view-active enterprise-nav-wrapper"
              : "enterprise-nav-wrapper"
          }
        >
          <EnterpriseNav
        ...
        <section
          className={`canvas-section ${mobileView === "canvas" ? "mobile-view-active" : ""}`}
        ...
        >
          ...
          <div
            ...
            aria-label={`七幕故事进度，当前第 ${currentAct || 0} 幕`}
          >
        ...
        <div
          className={
            mobileView === "events"
              ? "mobile-view-active info-panel-wrapper"
              : "info-panel-wrapper"
          }
        >
          <InfoPanel
            ...
            agentOutputs={scenario.agentOutputs}
            onSelectEvent={playback.controls.seek}
          />
        </div>
      </main>
...
export default function App() {
  ...
  if (error)
    return (
      <main className="error-state">
        ...
        <strong>场景无法加载</strong>
        <p>{error}</p>
        <p>请检查 preview.json 是否存在且包含七幕和 22 个事件。</p>
      </main>
    );
  if (!scenario)
    return (
  ...
  const navigateAtlasEntry = (entry: AtlasEntryRef) => {
    ...
    navigate(
      target === "live" ? "live" : target === "atlas" ? "atlas" : "preview",
    );
  };
  if (mode === "atlas")
    return (
