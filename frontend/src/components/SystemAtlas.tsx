...
export function SystemAtlas({onExit}:{onExit:()=>void}){
  ...
  const load=useCallback(async()=>{setLoading(true);setError('');try{const token=await atlasToken();const response=await fetch('/api/v1/system-atlas?view=aese',{headers:{Authorization:`Bearer ${token}`}});if(!response.ok){const body=await response.json().catch(()=>({}));throw new Error(body.error??`HTTP ${response.status}`)}setData(await response.json())}catch(reason){setError(reason instanceof Error?reason.message:'全景数据载入失败')}finally{setLoading(false)}},[]);
