...
export function LocationScene({building,projection,mission,onStart,onBack}:{building:GameBuilding;projection:GameProjection;mission?:GameWorkItem;onStart:()=>void;onBack:()=>void}){
 ...
 const items=building.kind==="office"?[
  ...
  {title:"开业财务中心",status:projection.finance_opening?.ready?"账套已启用":"等待资本到账",detail:projection.finance_opening?.ready?`${projection.finance_opening.book_code} · ${projection.finance_opening.accounting_standard} · ${projection.finance_opening.period_code}。这里是账套、凭证和报表的穿透入口，不在 AESE 复制财务系统。`:"实缴资本到账后，IAOS 将在同一事务建立财务组织、账套、会计期间和开业凭证。",source:projection.finance_opening?.evidence_ref??"IAOS Finance Opening",actions:projection.finance_opening?.ready?[
 ...
 return <section className={`gx-location-scene gx-location-${building.kind}`} aria-label={`${copy.title}室内场景`}>
  ...
  <div className="gx-room">
   ...
   {building.kind==="headquarters"&&<><div className="gx-hq-board"><Building2/><strong>{projection.brand.company_name}</strong><span>{organized?"首届管理团队组建中":"总部空间等待组织建立"}</span></div><div className={`gx-exec-seats ${organized?"unlocked":""}`}><i/><i/><i/><span>CEO · CFO · 工厂项目负责人</span></div><div className="gx-board-table"><UsersRound/><span>治理与经营会议桌</span></div>{projection.finance_opening?.ready&&financeItem&&<button type="button" className="gx-finance-center" aria-label="打开开业财务中心" onClick={()=>inspect(financeItem,2)}><WalletCards/><strong>开业财务中心</strong><span>账套 · 凭证 · 报表入口</span></button>}</>}
