...
const labels:Record<string,{title:string;description:string;action:string}>={
 ...
 "finance.organization.configure":{title:"建立开业财务组织",description:"由 IAOS 按已发布职责模板建立财务负责人、Controller、总账、资金、成本和内审岗位；空缺岗位不会伪造任职。",action:"配置财务组织"},
 "accounting.book.activate":{title:"启用会计账套与期间",description:"按企业法定主体启用 CAS-BE、CNY 本位币和首个开放期间，由财务 Controller 承担复核责任。",action:"启用账套"},
 "chart.of.accounts.activate":{title:"启用开业科目表",description:"启用银行存款与实收资本等开业必需科目，后续制造业科目按正式业务阶段扩展。",action:"启用科目"},
 "capital.contribution.post":{title:"过账实缴资本凭证",description:"通过受治理记账能力生成借：银行存款、贷：实收资本的双分录凭证；总账岗位空缺时会升级给创始治理者。",action:"生成并过账"},
 "finance.opening.readiness.evaluate":{title:"检查财务开业就绪",description:"由内部审计责任岗位检查财务组织、账套、开放期间、凭证与借贷平衡，不满足时拒绝推进。",action:"执行财务检查"},
