...
const item = (overrides: Partial<NonNullable<SiteInvestigationItem["observation"]>> = {}): SiteInvestigationItem => ({
  request: { proposal_id: "SITE-1" } as SiteInvestigationItem["request"], status: "observed", work_item_status: "completed",
...
describe("observed site assessment", () => {
  it("scores only delivered observations with explainable components", () => {
    const proposalSet = { proposals: [{ proposal_id: "SITE-1", display_name: "Agent 候选 A", estimated_amount: { likely: { value: "7500000.00" } }, estimated_schedule: { likely: "2026-11-01T00:00:00Z" } }] } as ProposalSet;
  ...
  it("hard-fails capacity before weighted score", () => {
    const result = assessObservedSites(requirement, null, [item({ electricity_kva: 1200 })], { cost: 100, schedule: 0, capacity: 0, control: 0 });
