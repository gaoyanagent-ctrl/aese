...
export function LiveSandbox({ layoutScenario, onModeChange }: { layoutScenario: SandboxScenario; onModeChange: (mode: 'preview' | 'live') => void }) {
  ...
  return <div className="app-shell live-shell">
    <ControlBar scenarioName="华辰苏州基地 · 在线企业沙盘" currentTime={new Date(snapshot.observed_at).toLocaleString('zh-CN', { hour12: false })}
      ...
      mode="live" onModeChange={onModeChange} sourceStatus={statusLabel} onRefresh={() => { void refresh().catch(() => undefined); }} onReconnect={() => setGeneration((v) => v + 1)} />
