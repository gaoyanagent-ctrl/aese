...
count=0
atlas_files() {
  if [ -n "${ATLAS_UPDATE_FILE:-}" ]; then
    [ -f "$ATLAS_UPDATE_FILE" ] || { echo "atlas update file not found: $ATLAS_UPDATE_FILE" >&2; return 1; }
    jq -r --arg file "$ATLAS_UPDATE_FILE" '[(.occurred_at//""),.update_key,$file]|@tsv' "$ATLAS_UPDATE_FILE"
    return
  fi
  find atlas-updates -maxdepth 1 -name '*.json' ! -name schema.json -print0 | while IFS= read -r -d '' file; do jq -r --arg file "$file" '[(.occurred_at//""),.update_key,$file]|@tsv' "$file"; done
}
while IFS= read -r file; do
  ...
  count=$((count+1))
done < <(atlas_files | sort -k1,1 -k2,2 | cut -f3-)
