...
## 2026-07-27 - M9 World 事实来源与案件投影修正

- 变更：DES-027 新增 D27；企业成立 World 不再固定展示 `HCTM-SZ-MFG` 为当前案件法人，
  而是优先读取 IAOS 拟设名称、登记法人编码、账户、认缴、实缴和预算字段，并逐项标记
  IAOS 已提交事实、设立案输入或 AESE 华辰场景假设。
- 原因：旧确定性基线与新交互式 IAOS trace 混合展示，使用户误以为没有输入的场景编码
  和金额是本次流程产生的事实。
- 影响：登记前显示拟设企业名称和“待登记编码”，登记后显示 IAOS 生成的法人编码；
  无案件或字段尚未提交时仍可解释世界基线，但不会冒充 IAOS 台账。
