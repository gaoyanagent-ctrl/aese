...
`PLAN-M9-NATIVE-001` 已因通用平台资产缺口恢复为 active remediation。专用状态机、
事务、身份、G1–G7、World Bridge 与 trace 证据保留；但只有资产真实注册到通用
Semantic/Entity/Capability/Process/Policy Runtime、十工作区与 Agent 操作层可用后，
才允许重新关闭 M9N。
...
## 36. M9 已交付范围（M9N 平台化验收重开）

以下业务范围已有专用闭环证据，但不能据此推导 M9N 通用平台注册完成。M9N 当前状态
以顶部里程碑表和 `PLAN-M9-NATIVE-001` 为准；只有 D19 通用资产与工作区验收完成后，
本节才可恢复为“已完成范围”。
