...
| M9N IAOS-native Incorporation Closed Loop | 用三层语义、正式身份、Runtime Artifact、Capability/Process/Approval、持久工作项和 World Bridge 重建 M9 真实闭环 | Completed | Runtime 2.6.0；`genesis-m9@1.0.2` 三包 Edition；30 个 Capability；23 个显式工作项（含 5 个财务开业节点）、G1–G7、三个 World wait、五次 Agent Run/五 Agent；设立案件权威表、Entity 投影与运行契约字段一致性门已验证 |
