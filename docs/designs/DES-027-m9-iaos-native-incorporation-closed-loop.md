...
### D20 — AESE 逐步骤 IAOS 可解释追踪

AESE incorporation campaign 的 World frame 与 IAOS Process transition 不是一一对应。
当前 8 个 World frame 聚合 IAOS 正常链的 15 次 Capability transition。两端必须通过
版本化的 step trace contract 建立显式关系，不得按数组下标、显示标题或时间邻近度猜测。

每个 AESE step trace 至少包含：

- `step`、`phase`、`causation_id` 和 `iaos_cursor`；
- `process_key`、`process_version`、`process_run_id` 和本步骤覆盖的状态节点；
- 一个或多个 `capability_key + capability_version`；
- acting principal、岗位、Mandate、Decision 和 Approval；
- 受影响 Entity code 与稳定业务记录引用；
- Journal、Outbox、Intent、Observation、CommittedOutcome 和 Discrepancy 引用；
- `tenant_id`、`case_code`、`world_run_id`、`correlation_id` 和 `idempotency_key`；
- 到 IAOS 设立案、Capability Studio、Process Studio 和 Entity Explorer 的稳定深链。

逐步投影以 IAOS trace 中的 Process transition 为主索引。World exchange 通过
`contract.payload.capability`、`causation_id` 和 `correlation_id` 关联；Decision、
Journal 和 Outbox 通过 capability、idempotency key、process run 和 correlation
关联。一个步骤可包含多个 Capability，但同一 Capability transition 只能归属一个步骤。
未匹配证据必须作为 `unmatched` 明示，禁止静默隐藏或错误挂到当前步骤。

AESE 当前步骤详情采用渐进披露：

1. 首屏显示 Process、Capability、执行主体、结果和 Entity 影响摘要；
2. World Bridge、治理证据和事务证据分别成组；
3. 原始 JSON 仅作为可展开的技术证据，不作为业务用户理解链路的唯一方式；
4. 切换时间线步骤时详情同步切换，整案 trace 不得伪装为当前步骤数据；
5. 375、768、1024 和 1440 宽度下不得产生页面级横向滚动，交互目标不小于 44px。

IAOS 企业生命周期页面必须接受 `step`、`capability`、`process_run` 和
`correlation` 深链参数。打开后自动加载案件，并在全局追踪中突出对应 transition；
平台工作室深链至少携带稳定 capability/process/entity code。数据库 UUID 只能作为
附加技术引用，不能成为跨仓关联主键。

验收要求：

- 8 个 AESE frame 均显示非空 Process/Capability 关联；
- 正常链 15 次 transition 恰好被覆盖一次；
- 登记、开户、任命三个 World Bridge 片段能分别看到 Intent、Observation 和
  CommittedOutcome；
- 任一步骤均可跳回同一 IAOS case，并携带步骤和能力上下文；
...
正常主线展开一主四子流程后必须生成 18 个工作项。除 15 次改变或确认生命周期状态的
正式 transition 外，`founder.resolution.prepare`、`registration.package.validate` 和
`initial.budget.prepare` 三个无状态变更能力也必须成为独立工作项，不能因为不改变
`lifecycle_state` 而被 UI 或 Runtime 跳过。工作项顺序必须由已发布 Process Definition
校验；`incorporation.case.open` 后首先解锁 `founder.resolution.prepare`，完成后才允许
进入 `founder.resolution.approve` / G1。

工作项序列不得在 API、前端或 Agent 代码中再次手写。Runtime 必须按
[ADR-005](../decisions/ADR-005-process-definition-as-runtime-source.md) 从已发布
Process Definition 和固定 version/artifact hash 编译工作项。恢复节点由分支条件激活，
不进入正常主线；发布门无法完成编译或发现顺序、Capability、Gate、参与主体不一致时
必须失败关闭。已开始 Process Run 继续绑定原 Artifact；只有尚未推进的案件可在显式
Runtime 升级时安全重建，已推进案件需要独立迁移预览、审批和审计。

业务用户不得直接编辑工作项 input JSON。工作项主操作必须打开由 Capability Input
Contract 解释出的业务表单弹窗，显示案件、节点、能力、审批门和执行主体，并以业务单位
接收输入：金额使用人民币元，提交时确定性转换为 minor unit；审批使用必填审批意见；
Agent task 使用可读的任务目标与限制说明。系统管理字段、correlation 和幂等键不向业务
用户索取。弹窗必须支持取消、字段就地校验、确认提交和移动端触控；原始 JSON 只允许在
专家证据视图只读展开。人工接管等会产生审计副作用的相似操作也应使用确认弹窗。
...
### D23 — Agent Task Runtime、工具与证据合同

`agent_task` 必须经过独立派发入口，不能继续复用“以 Agent 名义直接执行 Capability”的
快捷路径。首个纵向切片为节点 2 `founder.resolution.prepare`：

```text
Founder 创建设立案
→ 工作项 2 ready
→ 派发 incorporation-agent
→ 读取案件
→ 形成 governance_resolution 草稿
→ 调用 founder.resolution.prepare
→ 保存 Agent Run 和工具调用证据
→ 工作项 2 completed
→ 工作项 3 founder.resolution.approve waiting_approval
```

每个 Agent 必须有 tenant-scoped 的 service-only 主体、岗位、Mandate、Capability
白名单、工具白名单、Runtime 类型/端点、稳定职责说明、金额/有效期限制和升级主体。
每次派发必须在执行前写 `running` run，并在结束后写 `completed|failed`、业务任务说明、
Runtime、工具调用、结构化输出和错误。run ID 必须可从响应和案件 trace 关联。

当前 M9 交付使用 `deterministic + internal://incorporation-agent`，它是真实的受治理
Runtime 调度和证据链，但不是外部 LLM。界面必须明确显示“未启用外部模型”，不得要求
API Key，也不得把空 `model_provider/model_name` 描述为已经连接。以后接外部模型时必须
复用 IAOS AI Provider / AI Tool Registry 的密钥引用、HTTPS/host allowlist、SSRF
防护、超时、响应上限、逐次工具授权、输出 schema 校验和人工接管，禁止在场景包或
Agent 表保存明文密钥。

`incorporation-agent` 可为 G1 准备草稿，但不拥有 `approval.manage`，不能批准自身
产出。Founder 必须查看草稿并独立处理节点 3。任何 Runtime、Mandate、工具、Capability
或输出持久化失败都必须失败关闭；不得把无证据的运行标为 completed。

IAOS“Agent 组织”必须让客户无需读数据库即可查看 Agent 的岗位/Mandate、职责说明、
Runtime、是否接入模型、Capability/工具白名单、升级对象，以及当前案件每次 Agent Run
的输入、工具调用、输出和状态。专家配置合同见 IAOS
`DES-052-incorporation-agent-task-runtime.md`。

Capability Input Contract 由 IAOS/领域包版本化定义并随 Runtime Artifact 发布，不是
最终用户为每个任务上传的文件。Process 编译工作项时必须保存完整 `input_schema`
快照，弹窗按该合同显示业务字段和规则，服务端执行同一校验。节点 2 至少包含必填
`resolution_objective`（10–500 字符）、`key_proposals`（10–2000 字符）和选填
`risk_notes`（最多 1000 字符），不得再用无规则的单一“业务说明”代替合同。

节点 1 新建设立案必须收集案件名称、拟设企业名称、拟注册地址和拟经营范围；Runtime
生成单据编号、Asia/Shanghai 业务日期、初始状态和 payload，并在同一事务同步 canonical
