...
- [x] Z1 冻结 PlayerAccount、GenesisWorkspace、ProvisioningCheckpoint 和证据合同。
- [x] Z2 冻结 opaque tenant ID、workspace ID、case code 与 correlation 生成规则。
- [x] Z3 建立 IAOS/AESE 跨仓矩阵、权限矩阵、补偿矩阵和 failure injection 表。
- [x] Z4 验证 MiniMax endpoint/model/key health，禁止在日志输出密钥。
- [x] Z5 为 DES-029 建立 API、Schema 和 contract tests。

## 3. Z1 — IAOS Genesis Provisioning Saga

- [x] Z6 在独立 IAOS worktree 实现 `genesis.workspace.create/read` 资源。
- [x] Z7 新增控制平面 GenesisWorkspace/step 表和 owner/member 读取边界。
- [x] Z8 服务端编排 tenant create、plan、owner membership、M9 Runtime install。
- [x] Z9 完成 owner/runtime/World smoke check 后才激活 tenant。
- [x] Z10 实现 retry、幂等冲突、checkpoint 恢复和失败 tenant 隔离。
- [x] Z11 移除对全局固定 `founder-principal`、`platform_super_admin` 和 HCTM organization code 的新租户依赖。
- [x] Z12 两玩家/两租户、越权、重试和部分失败集成测试。

## 4. Z2 — AESE World 与 BFF

- [x] Z13 创建独立 World Run binding evidence，并在它 committed 后请求 IAOS 激活。
- [x] Z14 增加生产 workspace 列表/详情 BFF，不保存平台管理员凭据。
- [x] Z15 绑定 workspace、tenant、world run、creative job、case correlation。
- [x] Z16 验证 IAOS/AESE 重启、重复请求与 World 创建失败恢复。

## 5. Z3 — MiniMax Provider

- [x] Z17 实现 `MiniMaxProvider`、配置校验、HTTP 超时和响应上限。
- [x] Z18 版本化 prompt，要求严格 JSON naming proposal。
- [x] Z19 分离 reasoning、Schema 校验、一次 repair 和业务校验。
- [x] Z20 持久 CreativeJob 调用证据、token/latency/fallback reason。
- [x] Z21 实现单 Workspace 作业互斥、幂等、request-context 取消、一次重试和显式 deterministic fallback。
- [x] Z22 Provider 状态 API 区分 connected/degraded/fallback/not_configured。
- [x] Z23 真实 key smoke 与敏感日志测试。
...
- [x] Z27 实现五步创建向导和草稿自动保存。
- [x] Z28 实现八 checkpoint provisioning 进度、失败恢复和证据 ID。
- [x] Z29 workspace active 后获取 Founder tenant session 并进入 AI 身份工作室；旧管理员会话可 owner-scoped 刷新。
- [x] Z30 移除主路径对 tenant/case/auth_token URL 参数的依赖。
- [x] Z31 AI/fallback/Logo fallback 使用准确标签。

## 7. Z5 — 全链验收

- [x] Z32 从根地址创建 tenant A 并完成 M9。
- [x] Z33 创建/使用 tenant A、B 完成 Founder 选择、JWT claim、profile 和 M9 cross-read 隔离验证。
- [x] Z34 验证浏览器 token 无平台管理员权限。
...
- [x] Z37 更新 runbook/evidence/roadmap/code map/progress/Atlas 和两仓 revision。
...
- [x] Z42 把全部 23 个 M9 Work Item 逐章改造成董事会、政务、银行、财务、人才和经营会议专属 RPG 事件。
