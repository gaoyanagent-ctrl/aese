...
| M9N 可执行原子能力合同 | IAOS `docs/designs/DES-076-atomic-capability-library-and-business-authoring.md`、`docs/references/Atomic_ability.md`、`platform/internal/capability/atomic_catalog.go`；AESE PLAN-M9-NATIVE-001 §18 | AESE 不维护 Handler；Agent/Process/Bridge 调用已发布 Business Capability，由 IAOS 19 项 typed 原子能力在外层治理事务内执行；完整业务动作不得降级成原子能力。 |
