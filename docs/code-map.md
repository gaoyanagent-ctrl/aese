...
| M9N 平台基础语义消费 | IAOS `docs/designs/DES-071-semantic-governance-and-foundation-seed.md`、`platform/internal/semantic/foundation/foundation.v1.json`、Runtime 2.6.0、`docs/solutions/SOL-044-entity-semantic-inheritance-and-governed-record-browser.md`、`docs/solutions/SOL-046-incorporation-case-entity-field-parity.md`；AESE `docs/plans/2026-07-23-m9-iaos-native-incorporation-closed-loop.md` §20/§22/§24 | AESE/M9 只引用平台 Concept/Archetype；场景 seed 不得覆盖 foundation-owned 资产；新增前先执行目录检索并取得语义变更批准；权威表未分类业务列必须同时进入 Entity 编译合同和显式投影，否则平台包升级失败关闭 |
