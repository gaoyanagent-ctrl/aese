...
| M10 交互修订 active plan | `docs/plans/2026-08-01-m10-interactive-agent-plant-build.md`、`world-contracts/schemas/plant-build-interactive.schema.json`、`internal/plantbuild/interactive.go`、`internal/creative/minimax_provider.go::CompleteJSON`、`internal/httpapi/server.go`、`internal/iaosclient/client.go`；IAOS `incorporation/plant_entities.go`、`plant_entity_projections.go`、`api/plant_interactive.go`、`api/plant_process_run.go` | 已落地严格合同、MiniMax adapter、CreativeJob、财务快照、Agent/人工 Proposal、Review、World wait/Observation、权威推荐/Approval/正式选址与 UI；Agent Proposal 同时携带权威 Agent Run 并与 Capability/Process/Audit/Outbox 原子提交，九类业务对象为同事务只读 Entity 投影，`facility.plant.planning.v1` 串起人/Agent/World/审批；WBS/工程财务仍待实现 |
...
| AESE 后端部署与 Creative Provider 配置 | `scripts/deploy_aese_server.sh`、`.env.example`；安全加载权限为 0600 的本机 `.env`，校验 MiniMax 配置完整性，重建/重启 8090 并验证 provider 状态 |
...
| M9→M10 阶段交接与交互 Plant Build Play | `frontend/src/components/game/EnterpriseGenesisGame.tsx` 在 IAOS M9 终态显示携带 tenant/case/workspace 的交接主动作；`frontend/src/App.tsx` 负责双向深链；`frontend/src/components/world/PlantBuildPlay.tsx`、`frontend/src/world/plantBuild.ts`、`frontend/src/world/siteAssessment.ts`、组件/评分测试、`frontend/e2e/m9-m10-handoff.spec.ts` 与 `/#world-plant-build`。`siteAssessment.ts` 只作 Observation-only 预览；正式推荐/落地经 AESE 命名 BFF 调用 IAOS Capability |
...
| 场址推荐、审批与正式选择 | `frontend/src/components/world/PlantBuildPlay.tsx`、`frontend/src/world/plantBuild.ts`、`internal/httpapi/server.go`、`internal/iaosclient/client.go`；IAOS `plant_site_selection.go`、`incorporation/plant_approval.go`、`facility.site.selection.v1` | BFF 只采集人员选择和理由；IAOS 重算 `site-assessment-v1`、创建统一 Approval Request，approved 后由 `site.selection.formalize` 消费并写正式决定。浏览器不能指定审批人或直接写选择结果 |
| IAOS 交互权威切片 | `/iaos/iaos-go/platform/internal/api/plant_interactive.go`、`plant_site_selection.go`、`plant_process_run.go`：只读财务快照、Requirement/Proposal/Review、Investigation/World Observation、推荐/审批/正式选址、单一 Process Run、RLS、Capability 写门、幂等、Audit/Outbox；尚未包含 WBS/合同/施工/工程财务全链 |
