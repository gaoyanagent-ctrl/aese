...
| M9 IAOS 原生真实闭环设计 | `docs/designs/DES-027-m9-iaos-native-incorporation-closed-loop.md`；D19 通用资产可发现性、D20 逐步骤追踪、D21 可解释合同、D22 持久工作项与逐节点参与、D23 Agent Runtime/工具/运行证据及外部模型边界、D24 配置驱动审批路由/工作分发/通知、D25 Entity 生命周期与正式审批单一语义、D26 页面功能说明与配置依赖图；通用实现见 IAOS DES-053/DES-060 |
...
| M9N World 事实来源投影 | `frontend/src/world/incorporation.ts::resolveIncorporationFacts`、`frontend/src/components/world/IncorporationPlay.tsx`、DES-027 D27 | IAOS committed 字段优先；登记前展示拟设名称；缺少案件事实时将固定编码和金额明确标为 AESE 场景假设 |
