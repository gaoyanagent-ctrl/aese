...
| Genesis Workspace BFF | `internal/genesisworkspace/`、`/api/aese/v1/genesis/workspaces`、`POST /api/aese/v1/genesis/workspaces/:workspace/session`；转发当前 Player session 到 IAOS DES-062 生产控制面，绑定 Workspace/Tenant/World/Case 并兑换 tenant-only owner session；loopback local adapter 只作开发 fallback |
