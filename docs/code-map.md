...
| M9 IAOS 原生真实闭环设计 | `docs/designs/DES-027-m9-iaos-native-incorporation-closed-loop.md`；已批准的三层语义、平台主体、Runtime Artifact、Capability/Process、菜单、Trace Spine、World Bridge 与十二项验收门 |
| M9 IAOS 原生真实闭环 active plan | `docs/plans/2026-07-23-m9-iaos-native-incorporation-closed-loop.md`；P0–P7、T1–T66、双仓所有权、依赖顺序与完成门 |
...
| M9N IAOS lifecycle 联动页面 | `frontend/src/world/incorporation.ts`、`frontend/src/components/world/IncorporationPlay.tsx`；`#world-incorporation?tenant=&case=&process_run=&world_run=&correlation=` | 保留确定性 World frame，同时持 token 消费 IAOS 持久 lifecycle/process projection，展示 Intent/Observation/CommittedOutcome/Discrepancy/lineage 并提供五参数双向深链 |
