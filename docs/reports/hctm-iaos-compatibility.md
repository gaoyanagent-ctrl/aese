...
| `purchase_order` | U | 无完整 header entity | 无供应商、ETA、状态合同 | 不进首个 tracer |
| `goods_receipt` | U/M | 只有 `purchase_receipt_line` | 无 receipt header，line 字段仅 product/qty/batch/warehouse | 不进首个 tracer |
| `inspection_order` | U/M | `iqc` 部分对应 | 可对应 inspection no/product/batch/sample/result/inspector/time；无通用 object/source/accepted/rejected/defect | M4 前单独映射，M3 不 apply |
